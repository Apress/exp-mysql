...
/*
  ...
  SYNOPSIS
    convert()
    COND * IN The lex structure to convert.
  ...
  RETURN VALUE
    ...
    Failed = 1
*/
int Expression::convert(THD *thd, COND *mysql_expr)
{
  COND *pcond = mysql_expr;
...
char *Expression::to_string()
{
  ...
  {
    strcat(str, " (");
    strcat(str, pcond->str_value.c_ptr());
    strcat(str, ")");
    pcond = pcond->next;
  }
  DBUG_RETURN(str);
...
int Expression::compare_join(expr_node *expr, TABLE *t1, TABLE *t2)
{
  ...
  */
  ...
  {
    int i;
    Field *fl = find_field(t1, (char *)((Field *)expr->left_op)->field_name);
    Field *fr = find_field(t2, (char *)((Field *)expr->right_op)->field_name);
    i = memcmp(fl->ptr, fr->ptr, fl->field_length < fr->field_length ? 
      fl->field_length : fr->field_length);
    DBUG_RETURN(i);
  }
  DBUG_RETURN(90125);
}
...
bool Expression::compare(expr_node *expr, TABLE *t1)
{
  ...
  */
      ...
      (expr->right_op->type() == Item::INT_ITEM))
  {
    ...
    if(expr->left_op->type() == Item::FIELD_ITEM)
    {
      f = find_field(t1, (char *)((Field *)expr->left_op)->field_name);
      if (f == NULL)
        DBUG_RETURN(-1);
      a = f->val_int();
    }
    else
      a = atol(expr->left_op->name);
    if(expr->right_op->type() == Item::FIELD_ITEM)
    {
      f = find_field(t1, (char *)((Field *)expr->right_op)->field_name);
      if (f == NULL)
        DBUG_RETURN(-1);
      b = f->val_int();
    }
    else
      b = atol(expr->right_op->name);
  ...
  */
           ...
           (expr->right_op->type() == Item::DECIMAL_ITEM))
  {
    ...
    if(expr->left_op->type() == Item::FIELD_ITEM)
    {
      ...
      str2my_decimal(E_DEC_FATAL_ERROR, expr->left_op->name, 
        strlen(expr->left_op->name), system_charset_info, a);
    if(expr->right_op->type() == Item::FIELD_ITEM)
    {
      f = find_field(t1, (char *)((Field *)expr->right_op)->field_name);
      if (f == NULL)
      ...
      str2my_decimal(E_DEC_FATAL_ERROR, expr->right_op->name, 
        strlen(expr->right_op->name), system_charset_info, b);
