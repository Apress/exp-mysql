...
int Attribute::index_of(char *table, char *value)
{
  ...
  while ((item = it++) && !found)
  {
    if (strcasecmp((char *)((Field *)item)->table_name, "?") == 0)
    {
      k = strcasecmp(((Item_field *)item)->table_name, 
          table);
    }
    else
    {
      k = strcasecmp(((Field *)item)->table->s->table_name.str, 
          table);
    }
...
int Attribute::hide_attribute(Item *item, bool hide)
{
  ...
  if (((Field *)item)->table != NULL)
    i = index_of(
      (char *)((Field *)item)->table->s->table_name.str, 
      (char *)((Field *)item)->field_name);
  else
    i = index_of(
      (char *)((Item_field *)item)->table_name, 
