...
int Attribute::index_of(char *table, char *value)
{
  ...
  while ((item = it++) && !found)
  {
    if ((strcasecmp(((Field *)item)->field_name, value) == 0) &&
       ((table == NULL) ||
       (strcasecmp(((Field *)item)->table->s->table_name.str, table) == 0)))
    {
      j = i;
      found = true;
    }
...
int Attribute::hide_attribute(Item *item, bool hide)
{
  ...
  i = index_of((char *)((Field *)item)->table->s->table_name.str, 
...
char *Attribute::to_string()
{
  ...
  while (item = it++)
  {
    ...
    {
      strcat(str, ((Item_field *)item)->table_name);
      strcat(str, ".");
    }
    strcat(str, ((Item_field *)item)->field_name);
    if (!j)
      j = 1;
  }
  DBUG_RETURN(str);
